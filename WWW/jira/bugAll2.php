...
@include_once('conToJira.php');
	...
	if (!empty($_POST["projCheck"]) ) {
		$projarray=$_POST['projCheck'];
		$projstr=implode(",",$projarray);
		$projfilter.="  and  project.ID in (".$projstr.") ";		
	}
	if (!empty($_POST["prioCheck"])) {
		$prioarray=$_POST['prioCheck'];
		$priostr=implode(",",$prioarray);
		$issuefilter.=" and jiraissue.priority in (".$priostr.") ";
	}
	if ($_POST['startdate']!=null) {
		$startdate=date("Y-m-d",strtotime($_POST['startdate']));
		$issuefilter.=" AND DATE_FORMAT(jiraissue.CREATED,'%Y-%m-%d')>='".$startdate."' ";
	}
	if ($_POST['completedate']!=null) {
		$completedate=date("Y-m-d",strtotime($_POST['completedate']));
		$issuefilter.=" AND DATE_FORMAT(jiraissue.CREATED,'%Y-%m-%d')<='".$completedate."' ";
	}	
...
(SELECT jiraissue.PROJECT,COUNT(*) as total FROM jiraissue 
	WHERE jiraissue.issuetype=1 ".$issuefilter."
...
(SELECT pidx,COUNT(*) as sumprd from 
	(SELECT jiraissue.ID as issueid,jiraissue.PROJECT as pidx from jiraissue
		where jiraissue.issuetype=1 ".$issuefilter.") x 
	LEFT JOIN
...
(SELECT jiraissue.PROJECT as pidc,customfieldvalue.STRINGVALUE as pre,COUNT(*) as sumpre from jiraissue
	...
	where jiraissue.issuetype=1 
	 AND customfieldvalue.CUSTOMFIELD =10124 AND customfieldvalue.STRINGVALUE in (10121,10300) 
	 ".$issuefilter."
...
(select nodeassociation.SOURCE_NODE_ID,nodeassociation.SINK_NODE_ID,projectcategory.cname from nodeassociation 
	...
	where project.pname not LIKE '%关闭%' AND project.pname not LIKE '%未立项%' ".$projfilter."
