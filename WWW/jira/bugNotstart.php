...
$bugNotstart="SELECT project.ID as PID,project.pname as pname,project.LEAD as leader,project.pkey as pkey,aa.total,bb.sumprd,cc.sumpre,dd.valid,ee.notclosed,ff.resolved,gg.uname,hh.cname from project 
LEFT JOIN
...
(select app_user.user_key,cwd_user.display_name as uname from app_user
	LEFT JOIN
	cwd_user on app_user.lower_user_name=cwd_user.lower_user_name
	group by app_user.user_key) gg on project.LEAD=gg.user_key
LEFT JOIN
(select nodeassociation.SOURCE_NODE_ID,nodeassociation.SINK_NODE_ID,projectcategory.cname from nodeassociation 
	...
	where project.pname LIKE '%未立项%' 
order by project.ID DESC";
	
	$result=$conjira->query($bugNotstart);
	if(!$result)
	{
	...
	{
		$listNotstart[] = $result_array;
	}	

	echo json_encode($listNotstart);
