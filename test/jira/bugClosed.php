...
$bugClosed="SELECT project.ID as PID,project.pname as pname,project.LEAD as leader,project.pkey as pkey,aa.total,bb.sumprd,cc.sumpre,dd.valid,ee.notclosed,ff.resolved,gg.uname,hh.cname from project 
LEFT JOIN
...
(select nodeassociation.SOURCE_NODE_ID,nodeassociation.SINK_NODE_ID,projectcategory.cname from nodeassociation 
	...
	where project.pname LIKE '%关闭%' 
order by project.ID DESC";
	
	$result=$conjira->query($bugClosed);
	if(!$result)
	{
	...
	{
		$listClosed[] = $result_array;
	}	

	echo json_encode($listClosed);
