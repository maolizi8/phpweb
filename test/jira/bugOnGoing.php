...
(select project.ID as PID,project.pname as pname,project.LEAD as leader,project.pkey as pkey,COUNT(*) as total from project 
	...
	AND DATE_FORMAT(jiraissue.CREATED,'%Y-%m-%d')>='2016-10-01' 
		AND DATE_FORMAT(jiraissue.CREATED,'%Y-%m-%d')<='2016-10-18' 
			AND project.pname not LIKE '%关闭%' AND project.pname not LIKE '%未立项%' 
...
order by aa.PID DESC";
	...
	$result=$conjira->query($bugOnGoing);
	if(!$result)
	{
